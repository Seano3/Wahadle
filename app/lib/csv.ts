...
import { getUnits, setUnits, isUnitsLoaded } from "./unitsStorage";
...
export async function loadUnits(): Promise<UnitRow[]> {
  if (isUnitsLoaded()) {
    ...
    console.log("Loading units from Supabase Edge Function");
    ...
    console.log(`Loaded ${units.length} unit variants from Supabase Edge Function`);

    setUnits(units.filter(u => !!u["Unit Name"]));
